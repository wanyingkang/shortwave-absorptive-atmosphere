...
! ==================================================================================
   ...
   use astronomy_mod,         only: astronomy_init, diurnal_solar, daily_mean_solar, diurnal_exoplanet
...
logical :: use_daily_mean_insol = .true. ! wykang
logical :: use_exo_diurnal = .false. ! wykang
real    :: dt_rad_avg     = -1
...
namelist/two_stream_gray_rad_nml/ solar_constant, del_sol, &
           ...
           use_daily_mean_insol,& ! wykang
           use_exo_diurnal,& ! wykang
...
if (do_seasonal) then
  ...
  ! wykang add option to turn off diurnal cycle
  if (use_daily_mean_insol) then
      call daily_mean_solar(lat,time_since_ae,coszen,fracsun,rrsun)
  else
  if(use_time_average_coszen) then
     ...
     !wykang: add option for using diurnal_exoplanet
     if (use_exo_diurnal) then
         if (int(day_in_s) .ne. int(seconds_per_sol)) then
            dt_rad_radians=dt_rad_radians*day_in_s/seconds_per_sol
         end if
         call diurnal_exoplanet(lat, lon, Time_diag, coszen, fracsun, rrsun, dt_rad_radians)
     else
     ...
     !wykang: add option for using diurnal_exoplanet
     if (use_exo_diurnal) then
         call diurnal_exoplanet(lat, lon, Time_diag, coszen, fracsun, rrsun)
     else
